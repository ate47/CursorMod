...
@Mod(modid = CursorMod.MOD_ID, name=CursorMod.MOD_NAME, version=CursorMod.MOD_VERSION,
	guiFactory = CursorMod.MOD_GUI_FACTORY)
public class CursorMod {
	...
	public static void registerCursor(CursorType... cursorTypes) {
		...
		} else logger.log(Level.SEVERE, "Can't register new CursorType after FML Post Initialization.");
	}
	/**
	 * Save mod config
	 */
    ...
    public static void syncConfig() {
		for (CursorType type: cursors.keySet()) {
			try {
				cursors.put(type, gson.fromJson(config.getString(type.getConfigName(), "cursors", 
						gson.toJson(cursors.getOrDefault(type, cursors.get(CursorType.POINTER))), ""), CursorConfig.class));
			} catch (Exception e) {
    ...
    public void onConfigChanged(OnConfigChangedEvent ev) {
    	if(ev.modID.equals(CursorMod.MOD_ID)) syncConfig();
    }
	@SubscribeEvent
	...
	public void onDrawScreen(DrawScreenEvent ev) {
			...
			for (Field f: fa) {
				try {
					...
					if(o==null) continue;
						...
						if(isHover(ev.mouseX, ev.mouseY, selectZone.xPosition, selectZone.yPosition, selectZone.width, 
								selectZone.height) && selectZone.enable)
		...
		if (gui instanceof GuiContainer) {
			...
			IChatComponent ichatcomponent = gui.mc.ingameGUI.getChatGUI().getChatComponent(Mouse.getX(), Mouse.getY());
	        if (ichatcomponent != null && ichatcomponent.getChatStyle().getChatClickEvent() != null)
				newCursorType = CursorType.HAND;
		}
		for (CursorType cursorType: cursors.keySet())
			if(cursorType.getCursorTester()!=null && cursorType.getCursorTester().testCursor(newCursorType, gui, ev.mouseX, ev.mouseY, 
					ev.renderPartialTicks)) {
		...
		while (iterator.hasNext()) {
			...
			gui.mc.getTextureManager().bindTexture(new ResourceLocation("textures/gui/click_"+(2-cursorClick.getTime()/4)+".png"));
	...
	public void onMouseInput(MouseInputEvent ev) {
		if(Mouse.isButtonDown(0)) {
			ScaledResolution scaledResolution = new ScaledResolution(ev.gui.mc, ev.gui.mc.displayWidth, ev.gui.mc.displayHeight);
